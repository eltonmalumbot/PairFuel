...
function inviteCode() {
  return randomBytes(5).toString("base64url").replace(/[-_]/g, "").slice(0, 6).toUpperCase();
}
...
async function createInvite() {
  ...
  if (existing.length) redirect("/dashboard?tab=together&message=Already%20connected%20to%20a%20partner.");
  ...
  for (let attempt = 0; attempt < 5; attempt++) {
    ...
    try {
      ...
      revalidatePath("/dashboard");
      redirect("/dashboard?tab=together&message=Invite%20code%20created.");
    } catch (error: any) {
...
async function disconnectPartner() {
  ...
  const user = await currentUser();
  const sql = db();
  await sql`DELETE FROM pairfuel_partnerships WHERE user_a_id=${user.id} OR user_b_id=${user.id}`;
  revalidatePath("/dashboard");
  redirect("/dashboard?tab=together&message=Partner%20disconnected.");
}
...
async function updatePrivacy(fd: FormData) {
  ...
  revalidatePath("/dashboard");
  redirect("/dashboard?tab=settings&message=Privacy%20settings%20saved.");
}

async function updateGoals(fd: FormData) {
  "use server";
  const user = await currentUser();
  const sql = db();
  ...
  revalidatePath("/dashboard");
  redirect("/dashboard?tab=goals&message=Goals%20saved.");
}
...
export default async function Dashboard({ searchParams }: { searchParams: Search }) {
  ...
  return <main className="shell">
    ...
    <section className="dashboard">
      ...
      {tab === "history" && <div className="panel"><h2>Calorie history</h2><p className="muted">Latest 100 entries are shown here; full database history is retained.</p><div className="list">{foodHistory.map((food: any) => <div className="row" key={food.id}><div><b>{food.food_name}</b><div className="muted">{food.meal} · {formatJakartaDateTime(food.logged_at)} WIB</div></div><div>{food.calories} kcal</div></div>)}</div></div>}

      {tab === "fasting" && <div className="grid2"><form action={startFast} className="panel"><h2>Start a fast</h2><label className="field">Schedule<select name="target" defaultValue={String(profile.fasting_preset || "16:8").split(":")[0]}><option value="12">12:12</option><option value="14">14:10</option><option value="16">16:8</option><option value="18">18:6</option></select></label><button className="button">Start fasting</button><br/><br/><button formAction={endFast} className="ghost">End current fast</button></form><div className="panel"><h2>Fasting history</h2><div className="list">{fasts.map((fast: any) => <div className="row" key={fast.id}><span>{fast.target_hours}h target</span><span>{fast.ended_at ? "Completed" : "Active"}</span></div>)}</div></div></div>}
      ...
      {tab === "together" && <><StoryShare showTogether={Boolean(partner)} /><div className="grid2">{partner ? <><div className="panel"><h2>Your partner</h2><p className="muted">Only metrics your partner chose to share are shown.</p>{partner.share_calories && <div className="row"><span>Calories today</span><b>{partner.today?.calories || 0} kcal</b></div>}{partner.share_macros && <div className="row"><span>Protein today</span><b>{Math.round(Number(partner.today?.protein || 0))} g</b></div>}{partner.share_water && <div className="row"><span>Water</span><b>{partner.water?.amount_ml || 0} ml</b></div>}{partner.share_weight && <div className="row"><span>Weight</span><b>{partner.weight || "—"} kg</b></div>}{partner.share_weight_change && <div className="row"><span>Weight change</span><b>{partner.weightChange == null ? "—" : `${partner.weightChange > 0 ? "+" : ""}${partner.weightChange.toFixed(1)} kg`}</b></div>}{partner.share_fasting && <div className="row"><span>Latest fast</span><b>{partner.fast ? `${partner.fast.target_hours}h ${partner.fast.ended_at ? "done" : "active"}` : "—"}</b></div>}</div><form action={disconnectPartner} className="panel"><h2>Together</h2><div className="notice">Connected. PairFuel keeps each person’s data separate and respects sharing controls.</div><br/><button className="ghost">Disconnect partner</button></form></> : <><div className="panel"><h2>Invite your partner</h2>{invite ? <><p>Your 7-day invite code:</p><div className="stat"><b>{invite.code}</b></div></> : <form action={createInvite}><button className="button">Generate invite code</button></form>}</div><form action={acceptInvite} className="panel"><h2>Join a partner</h2><label className="field">Invite code<input name="code" maxLength={6} required /></label><button className="button">Connect</button></form></>}</div></>}

      {tab === "goals" && <form action={updateGoals} className="panel"><h2>Profile & goals</h2><p className="muted">Set targets that match your own plan. PairFuel is a tracker, not medical advice.</p><label className="field">Display name<input name="displayName" defaultValue={profile.display_name} required /></label><div className="grid2"><label className="field">Calories target<input name="calorieTarget" type="number" min="1200" defaultValue={profile.calorie_target} required /></label><label className="field">Protein target (g)<input name="proteinTarget" type="number" min="0" defaultValue={profile.protein_target} required /></label><label className="field">Carbs target (g)<input name="carbTarget" type="number" min="0" defaultValue={profile.carb_target} required /></label><label className="field">Fat target (g)<input name="fatTarget" type="number" min="0" defaultValue={profile.fat_target} required /></label><label className="field">Water target (ml)<input name="waterTarget" type="number" min="500" step="50" defaultValue={profile.water_target} required /></label><label className="field">Goal weight (kg)<input name="goalWeight" type="number" min="1" step="0.1" defaultValue={profile.goal_weight || ""} /></label></div><label className="field">Default fasting plan<select name="fastingPreset" defaultValue={profile.fasting_preset}><option>12:12</option><option>14:10</option><option>16:8</option><option>18:6</option></select></label><button className="button">Save goals</button></form>}
